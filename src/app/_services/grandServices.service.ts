...
export class GrandServicesService {
  ...
  getLastMinute(): Observable<GrandService[]> {
    const addSpecificFields = '?isPublished=1&random=3&tags=Last Call';
    return this.httpclient.get<GrandService[]>(this.apiURL + addSpecificFields);
  }

  getByTag(tag: string): Observable<GrandService[]> {
    const addSpecificFields = '?isPublished=1&tags=' + tag;
    return this.httpclient.get<GrandService[]>(this.apiURL + addSpecificFields);
  }
